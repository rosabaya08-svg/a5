      ...
      {[content.heroBanner, ...content.promoBanners].map((banner) => (
        <article key={banner.id} className="overflow-hidden rounded-md border border-slate-200 bg-white">
          ...
          <div className="grid gap-2 p-4 text-sm text-slate-600">
            ...
            <div className="flex justify-between"><span>상태</span><strong className="text-slate-950">approved mock</strong></div>
          </div>
        </article>
      ))}
    </section>
...
function ContentSlotsTable({ slots, type }: { slots: MarketingSlot[]; type?: "video" | "banner" }) {
  ...
  return (
    <DataTable
      columns={["소재", "노출 위치", "대상", "기간", "상태", "소유자", "성과 mock"]}
      rows={rows}
...
export async function AdminBannerManagementPage() {
  ...
  return (
    <AdminContentShell
      title="광고 배너 관리"
      subtitle="메인 배너, 기획전 배너, 팝업 배너의 노출 순서, 기간, 대상, 링크를 mock으로 검토합니다."
    >
      <FilterBar
        title="배너 필터"
...
export async function AdminVideoManagementPage() {
  ...
  return (
    <AdminContentShell
      title="광고 영상 관리"
      subtitle="영상 업로드는 Storage Blaze 전까지 보류하고, 썸네일/승인/노출 위치만 mock으로 관리합니다."
    >
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_360px]">
        <article className="rounded-md border border-slate-200 bg-white p-5">
          <div className="grid aspect-video place-items-center rounded-md bg-slate-950 text-center text-white">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-rose-300">video placeholder</p>
...
export function AdminHomeEditorPage() {
  ...
  return (
    <AdminContentShell title="메인 쇼핑몰 홈 편집" subtitle="배포 쇼핑몰형 홈 섹션 편성을 mock으로 검토합니다.">
      <section className="grid gap-3">
        {sections.map((section, index) => (
          <article key={section} className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-slate-200 bg-white p-4">
            <div>
              <p className="text-xs font-black uppercase text-slate-500">section {index + 1}</p>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800">노출 mock</span>
            </div>
          </article>
        ))}
      </section>
...
export function AdminExhibitionsPage() {
  return (
    <AdminContentShell title="기획전 관리" subtitle="브랜드별 기획전, 추천 상품 편성, 승인 상태를 mock으로 확인합니다.">
      <DataTable
          ...
          <article key={profile.productId} className="overflow-hidden rounded-md border border-slate-200 bg-white">
            ...
            <div className="p-4">
              ...
              <div className="mt-3 flex items-end justify-between gap-3">
                <strong className="text-xl text-rose-600">{product ? formatCurrency(product.price) : "가격 mock"}</strong>
    ...
    <CompanyContentShell title="영상 광고 소재 제출" subtitle="영상 파일 업로드는 보류하고 썸네일, 노출 위치, 승인 상태만 mock으로 확인합니다.">
      <section className="grid gap-4 lg:grid-cols-[1fr_360px]">
        <article className="rounded-md border border-slate-200 bg-white p-5">
          <div className="grid aspect-video place-items-center rounded-md bg-slate-950 text-center text-white">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-rose-300">video material mock</p>
...
export function CompanyExhibitionApplyPage() {
  return (
    <CompanyContentShell title="기획전 참여 신청" subtitle="기획전 참여 상품, 할인율, 노출 기간을 mock으로 제출합니다.">
      <DataTable
