...
async function readCompanyRuntimeScope(): Promise<CompanyRuntimeScope> {
  ...
  const companyId = sessionCompanyId || missingCompanyScopeId;
...
export async function CompanyProductPreviewPage({ productId }: { productId?: string } = {}) {
  ...
  if (productId) {
    ...
    const isCompanyProduct = productRead.data.companyId === scope.companyId;
...
export async function CompanySalesPage() {
  ...
  return (
    <CompanyShell title="매출 현황" subtitle="확정 주문과 주문상품 기준 매출을 확인합니다.">
      ...
      <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex rounded-md border border-slate-200 px-4 py-3 text-sm font-normal text-slate-900"
          >
            PayUp 가맹점 관리
          </a>
          <Link href="/company/excel" className="inline-flex rounded-md border border-slate-200 px-4 py-3 text-sm font-normal text-slate-900">
            주문 CSV 다운로드
          </Link>
        </div>
      </section>
