...
const safetyBadges = ["Firebase live data", "Firestore connected", "PG module pending", "No real payment"];
  ...
  return (
    <section className={`rounded-md border p-3 text-xs leading-5 ${isFirestore ? "border-emerald-200 bg-emerald-50 text-emerald-950" : "border-amber-200 bg-amber-50 text-amber-950"}`}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <strong>{isFirestore ? "Firestore live data" : "mock fallback data"}</strong>
        <span className="rounded-full bg-white px-2.5 py-1 font-black">{source}</span>
      </div>
      {reason ? <p className="mt-2 font-bold">Fallback reason: {reason}</p> : null}
    </section>
  );
}
