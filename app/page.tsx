...
const entryCards = [
  ...
  },
  {
    ...
    eyebrow: "ORDER",
    title: "비회원 주문조회",
    description: "주문번호와 고객 정보로 주문 상태를 확인합니다.",
    cta: "주문조회",
  },
];
    ...
    <main className="min-h-screen bg-slate-950 px-4 py-10 text-white">
      <section className="mx-auto max-w-6xl">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-4 text-4xl font-black leading-tight md:text-6xl">산후조리원 객실 기반 QR 결제 커머스</h1>
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {entryCards.map((card) => (
            <Link key={card.href} href={card.href} className="rounded-md bg-white p-5 text-slate-950 shadow-xl transition hover:-translate-y-1 hover:shadow-2xl">
              ...
              <span className="mt-5 inline-flex rounded-md bg-slate-950 px-4 py-3 text-sm font-black text-white">{card.cta}</span>
            </Link>
          ))}
        </div>
      </section>
