...
const defaultA5PublicOrigin = "https://signage-ai-a5.co.kr";
...
export async function qrCreateHandler(request: HttpRequestLike, response: HttpResponseLike): Promise<void> {
  ...
  try {
    ...
    const qrDisplayExpiresAt = new Date(now.getTime() + 5 * 60 * 1000).toISOString();
    ...
    await db.runTransaction(async (transaction) => {
      transaction.set(qrRef, {
        ...
        qr_display_expires_at: qrDisplayExpiresAt,
...
function toPublicQrSession(documentId: string, data: Record<string, unknown>) {
  ...
  return {
    ...
    qrDisplayExpiresAt: toIsoString(data.qr_display_expires_at ?? data.qrDisplayExpiresAt),
