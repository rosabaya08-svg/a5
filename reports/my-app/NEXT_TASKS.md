...
2. Convert `data/admin/operations.ts` from static beta records to Firestore repository reads after scoped rules are verified.
3. Add server-side approve/reject/request-fix endpoints with `SUPER_ADMIN` claim verification and audit log writes.
4. Wire CMS slot writes to Firestore collections for banners, videos, GIF/media assets, brand grid, exhibitions, popups, and home sections.
5. Add payment event detail drill-down after official PG webhook signature verification is ready.
6. Keep real PG cancel/refund, settlement payout, Alimtalk, delivery tracking, and external inventory blocked.
...
3. Verify Firestore read results for `company_id=company-sanho-care` and `nursery_id=nursery-gangnam-01` after Cloudflare deploy.
4. Add automated repository smoke checks for `companies`, `products`, `order_items`, `inventory_movements`, `rooms`, `tablets`, and `qr_payment_sessions`.
5. Move dashboard metric cards from `mockApi` aggregate summaries to Firestore aggregate read models once indexes are confirmed.
...
## 2026-05-26 태블릿 이동 버튼 다음 작업

1. 배포 후 `/tablet`, `/tablet/products`, `/tablet/products/product-care-kit`, `/tablet/cart`, `/tablet/qr`에서 로고 클릭과 고스트 뒤로/앞으로 버튼을 브라우저로 확인한다.
2. A3 탭의 글라스모피즘 배경과 겹칠 때 버튼 대비가 낮으면 위치와 투명도를 조정한다.
3. 키오스크 브라우저에서 forward 히스토리가 없는 상태를 감지할 필요가 있으면 비활성 스타일을 추가한다.

## 2026-05-26 태블릿 쇼핑몰 홈/가격 분석 다음 작업

1. Cloudflare 배포 후 `/tablet`에서 어두운 개발용 설명 박스가 사라지고 쇼핑몰형 메인 배너가 바로 노출되는지 확인한다.
2. `/tablet/products`에서 `AI 분석` 버튼 클릭 시 할인 분석 팝업이 열리는지 확인한다.
3. 팝업의 정상가, 비교 기준가, 폐쇄몰가가 기업 어드민 등록 가격과 Firestore 상품 가격 구조와 일치하는지 검증한다.
4. 카드 클릭과 분석 버튼 클릭이 서로 충돌하지 않는지 태블릿 브라우저에서 육안 확인한다.
5. 개발자 진단 정보는 운영 전 접이식 영역으로 정리하고, 고객 첫 화면에서는 쇼핑몰 콘텐츠를 우선 노출한다.

## 2026-05-26 태블릿 주문/QR 화면 다음 작업

1. Cloudflare 배포 후 `/tablet/qr`에서 상단 어두운 제목/설명 구역이 완전히 사라졌는지 확인한다.
...
1. Deploy `qrCreate` and `qrExpire` after final Functions environment approval.
2. Verify live Cloudflare flow from cart to `/q/{shortCode}`.
