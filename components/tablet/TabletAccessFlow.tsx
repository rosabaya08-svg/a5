...
import { resolveTabletDeviceAccessByDeviceId } from "@/lib/firebase/tabletDeviceResolveClient";
...
function isValidTabletRoomOption(room: TabletNurseryRoomOption) {
  const name = String(room.roomName ?? "").trim();
  const number = String(room.roomNumber ?? "").trim();
  return Boolean(room.roomId && !isDeviceFallbackRoomName(name) && !isDeviceFallbackRoomName(number));
}
...
function readTabletRoomOptions(session: TabletNurseryAccessSession | null): TabletNurseryRoomOption[] {
  ...
  try {
    ...
    return Array.isArray(cache.rooms) ? cache.rooms.filter(isValidTabletRoomOption) : [];
  } catch {
    return [];
  }
}
...
function saveTabletRoomOptions(session: TabletNurseryAccessSession, rooms: TabletNurseryRoomOption[]) {
  ...
  const cache: TabletRoomOptionsCache = {
    ...
    rooms: rooms.filter(isValidTabletRoomOption),
...
function readAutoEntryDeviceIdFromUrl() {
  if (typeof window === "undefined") return "";
  ...
  return (
    ...
    params.get("a3DeviceId") ??
    params.get("tabletDeviceId") ??
    ""
  ).trim();
}

function stripAutoEntryParamsFromUrl() {
  if (typeof window === "undefined") return;

  const url = new URL(window.location.href);
  ["deviceId", "device_id", "a3DeviceId", "tabletDeviceId"].forEach((key) => url.searchParams.delete(key));
  const next = `${url.pathname}${url.search}${url.hash}`;
  window.history.replaceState(null, "", next || "/tablet/products");
}

async function applyTabletDeviceAutoEntry(deviceId: string) {
  const resolved = await resolveTabletDeviceAccessByDeviceId(deviceId);

  if (!resolved.profile || !resolved.room) {
    return {
      ok: false,
      message: resolved.error?.message ?? "Tablet device auto entry failed.",
    };
  }

  const accessSession = buildTabletNurseryAccess(resolved.profile);
  saveTabletNurseryAccess(accessSession);
  saveTabletRoomOptions(accessSession, resolved.rooms.length > 0 ? resolved.rooms : [resolved.room]);
  saveTabletRoomSession({
    nurseryId: accessSession.nurseryId,
    businessNo: accessSession.businessNo,
    businessName: accessSession.businessName,
    registeredAddress: accessSession.registeredAddress,
    roomId: resolved.room.roomId,
    roomName: resolved.room.roomName || normalizeRoomName(resolved.room.roomNumber),
    tabletId: resolved.room.activeTabletId,
    fixedLogin: true,
    updatedAt: new Date().toISOString(),
  });

  stripAutoEntryParamsFromUrl();
  return { ok: true, message: "" };
}

export function TabletAutoEntryProbe() {
  useEffect(() => {
    const deviceId = readAutoEntryDeviceIdFromUrl();
    if (!deviceId) return;

    let cancelled = false;

    void applyTabletDeviceAutoEntry(deviceId).then(() => {
      if (cancelled) return;
      window.dispatchEvent(new CustomEvent("a5-tablet-room-session-change"));
    });
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-slate-950 px-4 text-white">
        <section className="w-full max-w-sm rounded-md bg-white p-6 text-center text-slate-950 shadow-2xl">
          <p className="text-sm font-normal text-slate-500">태블릿 접근 확인 중</p>
          <h1 className="mt-2 text-2xl font-normal">조리원 로그인과 객실 선택이 필요합니다</h1>
        </section>
      </main>
    );
  }
...
export function TabletContextBadge() {
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-2 rounded-full bg-white/80 px-3 py-1 text-xs font-normal text-slate-950 ring-1 ring-white/60">
      ...
      {isUnlockOpen ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 px-4 backdrop-blur-sm">
          <form onSubmit={unlockRoomEdit} className="w-full max-w-sm rounded-md bg-white p-5 text-slate-950 shadow-2xl">
            <h2 className="text-2xl font-normal">객실 정보 수정</h2>
            <label className="mt-4 grid gap-2 text-sm font-normal">
              조리원 비밀번호
              <input
                type="password"
                value={editPassword}
                onChange={(event) => setEditPassword(event.target.value)}
                className="h-12 rounded-md border border-slate-200 px-3 text-base font-normal"
                autoFocus
              />
            </label>
            {editMessage ? <p className="mt-3 rounded-md bg-red-50 p-3 text-sm font-normal text-red-700">{editMessage}</p> : null}
            <div className="mt-5 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setIsUnlockOpen(false)} className="h-11 rounded-md bg-slate-100 text-sm font-normal text-slate-900">
                취소
              </button>
              <button type="submit" className="h-11 rounded-md bg-rose-600 text-sm font-normal text-white">
                확인
              </button>
            </div>
          </form>
...
export function TabletLoginPage() {
  ...
  useEffect(() => {
    ...
    if (deviceId) {
      ...
      void applyTabletDeviceAutoEntry(deviceId).then((result) => {
        setSaving(false);
        if (result.ok) {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-slate-950 px-4 py-10 text-white">
      <section className="w-full max-w-sm rounded-md bg-white p-6 text-slate-950 shadow-2xl">
        <h1 className="text-3xl font-normal">폐쇄몰 태블릿 로그인</h1>
        <form onSubmit={submitTabletLogin} className="mt-6 grid gap-4">
          <label className="grid gap-2 text-sm font-normal">
            ...
            <input
              ...
              className="h-12 rounded-md border border-slate-200 px-3 text-base font-normal outline-none focus:border-slate-950"
          ...
          {message ? <p className="rounded-md bg-blue-50 p-3 text-sm font-normal text-blue-800">{message}</p> : null}
          <button disabled={saving} type="submit" className="h-12 rounded-md bg-slate-950 text-sm font-normal text-white disabled:opacity-60">
            {saving ? "확인 중" : "폐쇄몰 열기"}
          </button>
        </form>
      </section>
  ...
  const refreshRoomOptions = useCallback(async (session: TabletNurseryAccessSession, preferredRoomId = "") => {
    ...
    if (!lookup.profile) {
      ...
      if (cachedRooms.length > 0) {
        const preferred = cachedRooms.find((room) => room.roomId === preferredRoomId);
        setSelectedRoomId(preferred?.roomId ?? cachedRooms[0].roomId);
      } else {
        setSelectedRoomId("");
      }
    ...
    const liveRooms = (lookup.rooms ?? []).filter(isValidTabletRoomOption);
    saveTabletNurseryAccess(nextSession);
    setNurserySession(nextSession);

    if (liveRooms.length === 0) {
      const cachedRooms = readTabletRoomOptions(nextSession);
      setRooms(cachedRooms);
      if (cachedRooms.length > 0) {
        const preferred = cachedRooms.find((room) => room.roomId === preferredRoomId);
        setSelectedRoomId(preferred?.roomId ?? cachedRooms[0].roomId);
    ...
    return (
      <main className="min-h-screen bg-slate-950 px-4 py-10 text-white">
        <section className="mx-auto max-w-md rounded-md bg-white p-5 text-slate-950 shadow-2xl">
          <h1 className="text-3xl font-normal">객실 변경 확인</h1>
          ...
          <form onSubmit={unlockRoomEdit} className="mt-5 grid gap-4">
            ...
            {editMessage ? <p className="rounded-md bg-red-50 p-3 text-sm font-normal text-red-700">{editMessage}</p> : null}
            <button type="submit" className="h-12 rounded-md bg-rose-600 text-sm font-normal text-white">
              확인
            </button>
          </form>
        </section>
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-4 py-10 text-white">
      <section className="mx-auto max-w-4xl rounded-md bg-white p-6 text-slate-950 shadow-2xl">
        <p className="text-xs font-normal tracking-[0.18em] text-rose-600">객실 설정</p>
        <h1 className="mt-2 text-4xl font-normal">객실 선택</h1>
        ...
        <form onSubmit={saveRoom} className="mt-6 grid gap-4">
          <div className="rounded-md border border-slate-200">
            ...
            >
              <span>
                <span className="block text-xs font-normal uppercase tracking-[0.12em] text-rose-600">객실 선택</span>
                <span className="mt-1 block text-lg font-normal text-slate-950">
                  {selectedRoom ? normalizeRoomName(selectedRoom.roomName || selectedRoom.roomNumber) : roomsLoading ? "객실 불러오는 중" : "객실을 선택해 주세요"}
                </span>
              </span>
              <span className="shrink-0 rounded-full bg-slate-950 px-3 py-1 text-xs font-normal text-white">
            ...
            {isRoomListOpen ? (
              <div className="border-t border-slate-200 p-3">
                <input
                  ...
                  className="h-11 w-full rounded-md border border-slate-200 px-3 text-sm font-normal outline-none focus:border-slate-950"
                />
                <div className="mt-3 max-h-[420px] overflow-y-auto rounded-md border border-slate-200">
                  {visibleRooms.length > 0 ? (
                    visibleRooms.map((room) => (
                      ...
                      >
                        <span>
                          <span className="block text-lg font-normal">{normalizeRoomName(room.roomName || room.roomNumber)}</span>
                          <span className="mt-1 block text-xs font-normal opacity-70">{room.floor || "층 미지정"}</span>
                        </span>
                        <span className="self-center text-xs font-normal opacity-70">{room.activeTabletId || "태블릿 미지정"}</span>
                      </button>
                    ))
                  ) : (
          ...
          {roomMessage ? <p className="rounded-md bg-amber-50 p-3 text-sm font-normal text-amber-900">{roomMessage}</p> : null}

          <button
            type="submit"
            disabled={!selectedRoom || roomsLoading}
            className="h-12 rounded-md bg-rose-600 text-sm font-normal text-white disabled:cursor-not-allowed disabled:bg-slate-300"
          >
            적용하고 폐쇄몰 열기
          </button>
        </form>
