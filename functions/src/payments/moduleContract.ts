...
export const firebasePaymentEndpoints: FirebasePaymentEndpointContract[] = [
  ...
  {
    name: "Payment return trace",
    method: "POST",
    functionName: "paymentsReturnTrace",
    owner: "firebase_functions",
    purpose: "Record masked browser return parameters from InnoPay for debug and reconciliation.",
    realPgRequirement: "Used after the browser returns from the provider module; no PG secret is required.",
  },
  {
  ...
  {
    name: "Payment log monitor",
    method: "GET",
    functionName: "paymentsLogMonitor",
    owner: "firebase_functions",
    purpose: "Normalize recent payment return traces, payment events, webhook events, and payment ledgers for super-admin monitoring.",
    realPgRequirement: "Reads server-owned logs only; no PG secret is returned to browser code.",
  },
];
...
export const firebasePaymentCollections: FirebasePaymentCollectionContract[] = [
  ...
  },
  {
    ...
    clientWrite: "denied",
    readableBy: ["SUPER_ADMIN"],
    purpose: "Masked PG browser return parameters for debugging Return URL and transaction lookup failures.",
  },
  {
