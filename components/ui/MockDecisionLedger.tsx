...
export function MockDecisionLedger({ decisions }: { decisions: JourneyDecision[] }) {
  return (
    <section className="rounded-md border border-slate-200 bg-white p-4">
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.08em] text-slate-500">결정 기록</p>
        <h2 className="mt-1 text-xl font-black text-slate-950">실운영 전 안전 선택 사항</h2>
      </div>
          ...
          <article key={decision.id} className="rounded-md border border-slate-200 p-3">
            ...
            <div className="mt-3 grid gap-3 text-sm md:grid-cols-2">
              <div className="rounded-md bg-emerald-50 p-3">
                <p className="text-xs font-bold uppercase tracking-[0.08em] text-emerald-800">안전 선택</p>
                <p className="mt-1 leading-6 text-slate-700">{decision.safeMockChoice}</p>
              </div>
              <div className="rounded-md bg-red-50 p-3">
                <p className="text-xs font-bold uppercase tracking-[0.08em] text-red-700">실운영 필요 조건</p>
