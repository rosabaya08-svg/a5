...
import { isLegacyInnopayEnabled, isLegacyInnopayProviderName } from "./providerPolicy";
import {
  requestPayupCancel,
  requestPayupKeyinApproval,
  requestPayupStandardApproval,
  type PayupCardInput,
  type PayupEnvironment,
} from "./payupClient";
import {
  InnopayRestClient,
...
export type PgProviderCandidate = "payup" | "infiny" | "toss" | "portone" | "kcp" | "nice" | "unknown";
...
export function resolveProviderCandidate(value?: string): PgProviderCandidate {
  ...
  if (provider === "payup" || provider === "payuppg") return "payup";
  if (isLegacyInnopayEnabled() && isLegacyInnopayProviderName(provider)) return "infiny";
...
export function getProviderAdapterSlot(providerName?: string): PgProviderAdapterSlot {
  ...
  return {
    ...
    supportedCandidates: ["payup", "toss", "portone", "kcp", "nice"],
    operationPlan: {
...
export async function confirmPaymentWithConfiguredProvider(input: ProviderConfirmInput): Promise<ProviderConfirmResult> {
  ...
  if (isLegacyInnopayProviderName(rawProvider) && !isLegacyInnopayEnabled()) {
    return blocked("confirmPayment", "Legacy InnoPay/Infiny is disabled for A5 production payment. Use Payup provider settings.");
  }
  ...
  if (candidate !== "unknown") {
    if (candidate === "payup") return confirmPayupPayment(input);
...
async function confirmPayupPayment(input: ProviderConfirmInput): Promise<ProviderConfirmResult> {
  const merchantId = input.merchantId || readEnv("PAYUP_MERCHANT_ID") || readEnv("PG_MERCHANT_ID");
  const apiKey = input.secretKey || readEnv("PAYUP_API_KEY") || readEnv("PAYUP_API_CERT_KEY") || readEnv("PG_SECRET_KEY");
  const card = input.payupCard;

  if (!merchantId) {
    return blocked("confirmPayment", "Payup merchantId is missing. No Payup API was called.");
  }

  if (!apiKey) {
    return blocked("confirmPayment", "Payup apiKey is missing. Store it as the encrypted company secret key or set PAYUP_API_KEY.");
  }

  if (input.transactionId) {
    const result = await requestPayupStandardApproval({
      merchantId,
      apiKey,
    ...
    if (!result.ok) {
      return blocked("confirmPayment", `Payup standard approval failed: ${result.code} ${result.message}`, "payup");
    }

    if (typeof result.amount === "number" && result.amount !== input.amount) {
      return blocked("confirmPayment", `Payup approved amount mismatch. server=${input.amount}, provider=${result.amount}`, "payup");
    }

    const approvedAt = new Date().toISOString();
    const transactionId = result.transactionId || input.transactionId;
    ...
    return {
      ...
      approval: {
        ...
        paymentKey: result.paymentKey || transactionId,
        transactionId,
        receiptUrl: result.receiptUrl || input.receiptUrl,
        realPgCalled: true,
        approvedAt,
        message: "Payup standard payment approval succeeded through the configured server adapter.",
      },
    };
  }
  ...
  if (!card) {
    return {
      ok: false,
      code: "PAYUP_TRANSACTION_ID_MISSING",
      message: "Payup transactionId is missing. No approval API was called and no card data was stored or logged.",
      candidate: "payup",
      operation: "confirmPayment",
      realPgCalled: false,
    };
  ...
  if (!isPayupKeyinEnabled()) {
    return blocked("confirmPayment", "Payup key-in approval is disabled. Use the Payup standard checkout transactionId flow.", "payup");
  }

  const result = await requestPayupKeyinApproval({
    merchantId,
    apiCertKey: apiKey,
    environment: input.environment,
    orderNumber: input.orderNo,
    amount: input.amount,
    itemName: input.itemName || `A5 order ${input.orderNo}`,
    userName: card.userName || input.customerName || "A5 Guest",
    card,
  });

  if (!result.ok) {
    return blocked("confirmPayment", `Payup key-in approval failed: ${result.code} ${result.message}`, "payup");
  }
  ...
  const approvedAt = new Date().toISOString();
  const transactionId = result.transactionId || result.paymentKey || `PAYUP-${input.orderNo}`;
  ...
  return {
    ...
    approval: {
      ...
      realPgCalled: true,
      approvedAt,
      message: "Payup key-in payment approval succeeded through the configured server adapter.",
    },
  };
}
...
export async function handleProviderWebhook(input: ProviderWebhookInput): Promise<ProviderActionResult> {
  ...
  if (isLegacyInnopayProviderName(readiness.provider) && !isLegacyInnopayEnabled()) {
    return blocked("handleWebhook", "Legacy InnoPay/Infiny webhook handling is disabled. Use Payup provider settings.");
  }
...
function blocked(operation: PgProviderOperation, message: string, providerName?: string): ProviderBlockedResult {
  const readiness = getPgServerReadiness();
  return {
    ok: false,
    code: "PG_PROVIDER_ADAPTER_NOT_IMPLEMENTED",
    message,
    candidate: resolveProviderCandidate(providerName ?? readiness.provider),
    operation,
    realPgCalled: false,
  };
}

function isPayupKeyinEnabled(): boolean {
  return ["1", "true", "yes", "on"].includes(readEnv("A5_ENABLE_PAYUP_KEYIN").toLowerCase());
}
...
async function callProviderPaymentAction(
  ...
  const runtimeConfig = await readFirestorePgRuntimeConfig();
  const readiness = getPgServerReadiness();
  const providerName = String(input.provider ?? readiness.provider ?? "").trim();
  const candidate = resolveProviderCandidate(providerName);

  if (isLegacyInnopayProviderName(providerName) && !isLegacyInnopayEnabled()) {
    return blocked(operation, "Legacy InnoPay/Infiny cancel/refund is disabled. Use Payup provider settings.", providerName);
  }

  if (operation === "cancelPayment" && candidate === "payup") {
    const merchantId = input.merchantId || readEnv("PAYUP_MERCHANT_ID") || readEnv("PG_MERCHANT_ID");
    const apiKey = input.secretKey || readEnv("PAYUP_API_KEY") || readEnv("PAYUP_API_CERT_KEY") || readEnv("PG_SECRET_KEY");
    const transactionId = input.paymentKey || "";
    const partial = Number.isFinite(input.totalAmount) && input.amount > 0 && input.amount < Number(input.totalAmount);

    if (!merchantId) return blocked(operation, "Payup merchantId is missing. No cancel API was called.", providerName);
    if (!apiKey) return blocked(operation, "Payup apiKey is missing. No cancel API was called.", providerName);
    if (!transactionId) return blocked(operation, "Payup transactionId is required for cancel.", providerName);

    const result = await requestPayupCancel({
      merchantId,
      apiKey,
      environment: input.environment,
      transactionId,
    ...
    if (!result.ok) {
      return blocked(operation, `Payup cancel failed: ${result.code} ${result.message}`, providerName);
    }
    ...
    return {
      ...
      message: result.message || "Payup cancel succeeded.",
      raw: result.rawMasked,
    };
  }
  ...
  if (candidate === "unknown" || providerName.toLowerCase() === "mock") {
    return blocked(operation, "PG_PROVIDER is mock or unknown. No real cancel/refund API was called.", providerName);
  }

  if (!endpoint) {
    return blocked(operation, "PG cancel/refund endpoint is missing. No PG API was called.", providerName);
  }

  if (!secretKey) {
    return blocked(operation, "PG_SECRET_KEY is missing from the Functions runtime. No PG API was called.", providerName);
  }

  if (!input.paymentKey) {
    return blocked(operation, "paymentKey is required for real PG cancel/refund.", providerName);
  }
  ...
  try {
    ...
    return blocked(operation, error instanceof Error ? error.message : `Unknown PG ${operation} error.`, providerName);
  }
}
...
async function readFirestorePgRuntimeConfig(): Promise<FirestorePgRuntimeConfig> {
  try {
    ...
    const [infinySnapshot, legacySnapshot] = isLegacyInnopayEnabled()
      ? await Promise.all([
    ...
    const data = {
      ...
      ...(infinySnapshot?.exists ? infinySnapshot.data() ?? {} : {}),
