...
export async function a4NurseryAutoSignupHandler(request: HttpRequestLike, response: HttpResponseLike) {
  ...
  const sourceProfile = mapSourceProfile(
    sourceNursery.doc,
  ...
  const scope = await resolveA5NurseryScope(targetDb, {
    requestedNurseryId: sourceProfile.nurseryId,
    businessRegistrationNo: sourceProfile.businessRegistrationNo,
    externalNurseryId: sourceProfile.externalNurseryId,
  });
  const profile = {
    ...sourceProfile,
    nurseryId: scope.nurseryId,
    businessRegistrationNo: scope.businessRegistrationNo || sourceProfile.businessRegistrationNo,
    businessRegistrationNoNormalized: scope.businessRegistrationNoNormalized || sourceProfile.businessRegistrationNoNormalized,
    externalNurseryId: scope.externalNurseryId || sourceProfile.externalNurseryId,
  };
