...
function directDocIds(normalized: string) {
  return unique([normalized, `a4-${normalized}`, `business-${normalized}`, `nursery-${normalized}`, `nursery-auto-${normalized}`, `a1-nursery-${normalized}`]);
}
...
async function readRoomsByNursery(nurseryId: string): Promise<TabletRoomOption[]> {
  ...
  return roomSnapshot.docs
    .map((doc): TabletRoomOption | null => {
      ...
      const roomName = displayRoomNumber(fieldString(data, "name", "room_name", "roomName")) || roomNumber;
...
export async function tabletNurseryLoginHandler(request: HttpRequestLike, response: HttpResponseLike) {
  ...
  const customToken = await getAdminAuth().createCustomToken(`nursery:${resolved.profile.nurseryId}`, {
    role: "NURSERY_ADMIN",
    nursery_id: resolved.profile.nurseryId,
    business_registration_no: normalizeBusinessNo(resolved.profile.businessRegistrationNo),
  });
