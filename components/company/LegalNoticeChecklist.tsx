  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-normal tracking-[0.12em] text-blue-600">법적 확인</p>
          <h2 className="mt-1 text-lg font-normal text-slate-950">법적 고지 / 금지상품 / KC 인증 체크</h2>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        <section className="rounded-md border border-amber-200 bg-amber-50 p-4 text-amber-950">
          <h3 className="font-normal">전문가 검토 필요</h3>
          <p className="mt-2 text-sm leading-6">
            시스템은 입력 누락과 명백한 금지 후보를 차단하지만, 실제 판매 가능 여부와 법률 판단은 운영자/전문가 검토 후 확정해야 합니다.
          </p>
        </section>
        <section className="rounded-md border border-slate-200 bg-slate-50 p-4">
          <h3 className="font-normal text-slate-950">공식 문서 확인 후보</h3>
          <ul className="mt-2 grid gap-2 text-sm text-slate-700">
            {legalReviewSources.map((source) => (
              <li key={source.url}>
                <a href={source.url} target="_blank" rel="noreferrer" className="font-normal text-blue-700 underline underline-offset-2">
                  {source.label}
                </a>
              </li>
            ))}
...
function ChecklistGroup({ title, items }: { title: string; items: ComplianceField[] }) {
  return (
    <div className="rounded-md border border-slate-100 bg-slate-50 p-3">
      <h3 className="font-normal text-slate-950">{title}</h3>
...
function RestrictedItemsPanel() {
  return (
    <section className="rounded-md border border-red-200 bg-red-50 p-4">
      <h3 className="font-normal text-red-950">판매 금지/제한 항목</h3>
...
function CertificationPanel() {
  return (
    <section className="rounded-md border border-amber-200 bg-amber-50 p-4">
      <h3 className="font-normal text-amber-950">KC 인증 / 증빙</h3>
