...
1. Cloudflare 배포 후 `/tablet`, `/tablet/products`, `/tablet/products/product-care-kit`, `/tablet/cart`, `/tablet/qr`에서 고스트 뒤로/앞으로 버튼이 터치 가능한지 확인한다.
2. A3 탭 내 글라스모피즘 배경과 겹칠 때 버튼 대비가 부족하면 위치를 좌측 하단에서 우측 하단 또는 중앙 하단으로 조정한다.
3. 태블릿 키오스크 모드에서 브라우저 히스토리 forward가 비활성처럼 보이면 `앞으로가기` 버튼의 비활성 상태 표시를 추가한다.

## 2026-05-26 태블릿 쇼핑몰 홈/가격 분석 다음 작업

1. Cloudflare 배포 후 `/tablet`에서 개발용 어두운 설명 박스가 보이지 않고 쇼핑몰 배너가 바로 첫 콘텐츠로 보이는지 확인한다.
2. `/tablet/products`에서 각 상품의 `AI 분석` 버튼을 눌러 할인 분석 팝업이 열리고, 카드 링크 이동이 함께 발생하지 않는지 확인한다.
3. 기업 어드민 상품 등록 화면에서 정상가, 비교 기준가, 폐쇄몰가 입력 필드를 Firestore 상품 `comparison` 구조와 완전히 맞춘다.
4. PG 공식 키 수령 후에도 가격 분석 팝업은 결제 금액 산정 근거 안내로 유지하고, 실제 결제 금액은 서버 재계산 결과만 사용한다.
5. 상품 카드의 개발자용 ID/status/source/seeded_at 정보는 운영 전 하단 접이식 진단 패널로 이동한다.
