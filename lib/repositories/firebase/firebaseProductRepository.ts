...
const preservedTestProductId = "product-test-1004";
...
function companyBusinessNoAliases(companyId: string) {
  ...
  if (companyId === preservedTestCompanyId || normalizedCompanyId === REGISTERED_CLOSED_MALL_BUSINESS_NO) {
...
function resolveProductCompanyId(documentId: string, data: DocumentData) {
  ...
  if (productId === preservedTestProductId || businessNo === REGISTERED_CLOSED_MALL_BUSINESS_NO) {
    return preservedTestCompanyId;
  }
...
function productMatchesCompanyScope(documentId: string, data: DocumentData, companyId: string) {
  ...
  const productId = asString(data.product_id ?? data.productId, documentId);
  if (companyId === preservedTestCompanyId && productId === preservedTestProductId) return true;
...
async function readDirectCompanyProductSnapshots(db: Firestore, companyId: string) {
  if (companyId !== preservedTestCompanyId) return [];

  const snapshot = await getDoc(doc(db, productCollection, preservedTestProductId));
...
async function readActiveProducts(filters?: ProductListFilters) {
  ...
  try {
    ...
    const directDocuments = filters?.companyId ? await readDirectCompanyProductSnapshots(db, filters.companyId) : [];
