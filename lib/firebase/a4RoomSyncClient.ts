...
import type { A4LocalRoomUpsertResponse, A4RoomSyncResponse } from "@/types/nursery";
...
export type SaveA5LocalRoomInput = {
  nurseryId: string;
  businessRegistrationNo?: string;
  roomId?: string;
  roomNumber: string;
...
function getLocalRoomUpsertUrl() {
  return (
    (process.env.NEXT_PUBLIC_A5_LOCAL_ROOM_UPSERT_URL ?? "").replace(/\/$/, "") ||
    `${getFunctionsBaseUrl()}/a4LocalRoomUpsert`
  );
}
...
export async function saveA5LocalRoomToFirestore(input: SaveA5LocalRoomInput): Promise<A4LocalRoomUpsertResponse> {
  const url = getLocalRoomUpsertUrl();

  if (!url || url === "/a4LocalRoomUpsert") {
    return {
      ok: false,
      error: {
        code: "A4_LOCAL_ROOM_UPSERT_URL_MISSING",
        message: "A5 local room upsert function URL is not configured.",
        httpStatus: 0,
      },
    };
  }
  ...
  try {
    ...
    const data = (await response.json()) as A4LocalRoomUpsertResponse;

    if (!response.ok && data.ok !== false) {
      return {
        ok: false,
        error: {
          code: "A4_LOCAL_ROOM_UPSERT_HTTP_ERROR",
          message: `HTTP ${response.status}`,
          httpStatus: response.status,
        },
      };
    ...
    return {
      ok: false,
      error: {
        code: "A4_LOCAL_ROOM_UPSERT_REQUEST_FAILED",
        message: error instanceof Error ? error.message : "Local room upsert request failed.",
        httpStatus: 0,
      },
    };
  } finally {
