...
const activeSmokeSteps = smokeSteps.filter((step) => !step.route.startsWith("/mock-ui"));
    ...
    <main className="min-h-screen bg-slate-100 px-4 py-6 text-slate-950 md:px-8">
      <div className="mx-auto grid max-w-7xl gap-5">
        <header className="rounded-md bg-slate-950 p-5 text-white">
          <p className="text-xs font-normal uppercase tracking-[0.14em] text-emerald-200">화면 점검 체크리스트</p>
          <h1 className="mt-3 text-4xl font-normal">localhost:3000 수동 클릭 순서</h1>
        ...
        <section className="grid gap-3 lg:grid-cols-2">
          {activeSmokeSteps.map((step, index) => (
            <article key={step.id} className="rounded-md border border-slate-200 bg-white p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-normal uppercase tracking-[0.08em] text-slate-500">{index + 1}단계</p>
                  <h2 className="mt-1 text-xl font-normal text-slate-950">{step.title}</h2>
                </div>
                <Link href={step.route} className="rounded-md bg-slate-950 px-3 py-2 text-xs font-normal text-white">
                  {step.route}
                </Link>
              </div>
              <ul className="mt-4 grid gap-2">
                {step.checks.map((check) => (
                  <li key={check} className="rounded-md bg-slate-50 px-3 py-2 text-sm font-normal text-slate-700">
                    {check}
                  </li>
                ))}
              </ul>
        ...
        <section className="rounded-md border border-slate-200 bg-white p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-xs font-normal uppercase tracking-[0.08em] text-slate-500">경로 목록</p>
              <h2 className="mt-1 text-xl font-normal text-slate-950">클릭 가능한 화면 점검 경로</h2>
            </div>
            <span className="rounded-full bg-red-50 px-2.5 py-1 text-xs font-normal text-red-700">
              수동 확인 전용
            </span>
          </div>
          <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
            {activeSmokeRoutes.map((route) => (
              <Link key={route.id} href={route.route} className="rounded-md bg-slate-50 p-3 hover:bg-white">
                <p className="font-normal text-slate-950">{route.route}</p>
