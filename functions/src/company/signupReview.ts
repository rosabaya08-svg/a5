...
import { A5_COMPANY_SOURCE_SITE, a5CompanyAuthClaims, a5MemberDocumentFields } from "../identity/source";
...
type CompanySignupRequestRecord = {
  ...
  representativeBirthDate?: unknown;
  representative_birth_date?: unknown;
  representativeNationality?: unknown;
  representative_nationality?: unknown;
  representativeGender?: unknown;
  representative_gender?: unknown;
  managerName?: unknown;
...
export async function companySignupReviewHandler(request: HttpRequestLike, response: HttpResponseLike): Promise<void> {
  ...
  const authProvisioning = action === "approve"
    ...
    : { status: "not_required" };
  const memberHubSync = action === "approve"
    ? await syncA5CompanyMemberToHub({
        companyId,
        businessNo: text(signup.businessRegistrationNumber) || text(signup.business_registration_number) || normalizedBusinessNo,
        companyName: companyName(signup) || companyId,
        managerEmail: text(signup.managerEmail) || text(signup.manager_email),
        managerName: text(signup.managerName) || text(signup.manager_name),
        managerPhone: text(signup.managerPhone) || text(signup.manager_phone),
        status: "active",
        approvalStatus: "approved",
        authUid: typeof (authProvisioning as { uid?: unknown }).uid === "string" ? (authProvisioning as { uid: string }).uid : undefined,
...
function buildRequestPatch(input: {
  ...
  return {
    status,
    ...a5MemberDocumentFields({ sourceSite: A5_COMPANY_SOURCE_SITE, memberType: "company" }),
    infiny_transfer_status: transferStatus,
    approved_company_id: input.companyId,
    approvedCompanyId: input.companyId,
    pg_provider: "payup",
...
function buildApprovedCompanyDocument(
  ...
  return {
    ...
    representative_birth_date: text(signup.representativeBirthDate) || text(signup.representative_birth_date),
    representative_nationality: text(signup.representativeNationality) || text(signup.representative_nationality),
    representative_gender: text(signup.representativeGender) || text(signup.representative_gender),
    ...
    pg_merchant_status: merchantStatus,
    payup_mid: merchantId,
    payup_mid_status: merchantStatus,
    ...taxableMerchantPolicy,
    pg_profile: {
      provider: "payup",
...
async function provisionCompanyAdminAuthUser(companyId: string, signup: CompanySignupRequestRecord, actor: AdminActor) {
  ...
  try {
    ...
    const customClaims = a5CompanyAuthClaims({
      companyId,
      businessNo: normalizeBusinessNo(text(signup.businessRegistrationNumber) || text(signup.business_registration_number)),
      ownerUid: user.uid,
    });
    ...
    await Promise.all([
      db.collection("companies").doc(companyId).set(
        {
          ...a5MemberDocumentFields({ sourceSite: A5_COMPANY_SOURCE_SITE, memberType: "company", ownerUid: user.uid }),
          auth_uid: user.uid,
          auth_email: email,
...
function auditMessageFor(action: CompanySignupReviewAction, companyId: string) {
  if (action === "approve") return `${companyId} 기업 상품등록 권한 승인이 완료되었습니다.`;
