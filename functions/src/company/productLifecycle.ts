...
export async function companyProductLifecycleHandler(
  ...
  if (!productId || !isLifecycleAction(action)) {
    sendJson(response, 400, {
      ok: false,
      error: {
        code: "COMPANY_PRODUCT_LIFECYCLE_REQUEST_INVALID",
  ...
  if (!token) {
    sendJson(response, 401, {
      ok: false,
      error: {
        code: "COMPANY_PRODUCT_LIFECYCLE_AUTH_REQUIRED",
  ...
  if (!claims) {
    sendJson(response, 403, {
      ok: false,
      error: {
        code: "COMPANY_PRODUCT_LIFECYCLE_FORBIDDEN",
        message: "The signed-in account cannot manage this product.",
      },
    });
    return;
  }
  ...
  if (!companyId || !canWriteCompanyScope(claims as A5AuthClaims, companyId)) {
    sendJson(response, 403, {
      ok: false,
      error: {
        code: "COMPANY_PRODUCT_LIFECYCLE_SCOPE_MISMATCH",
        message: "The signed-in company account cannot manage this product.",
      },
    });
    return;
  }
  ...
  const nextStatus =
    action === "archive" ? "archived" : action === "suspend" ? "paused" : "active";
  const nextRelatedStatus =
    action === "archive" ? "archived" : action === "suspend" ? "suspended" : "active";
  const nextModerationStatus =
    action === "restore" ? "registered" : action === "archive" ? "archived" : "restricted";
  ...
  const auditRef = db
    ...
    .doc(`company-product-lifecycle-${productId}-${Date.now()}`);
  ...
  if (supplySnapshot.exists) {
    batch.set(
      supplyRef,
      { ...lifecycleFields, status: nextRelatedStatus },
      { merge: true },
    );
  }
  ...
  batch.set(auditRef, {
    ...
    storefront_snapshot_status: "pending",
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Product lifecycle update failed.";
  ...
  let snapshotVersion: string | number | null = null;
  let snapshotError = "";
  try {
    const snapshot = await publishStorefrontRuntimeSnapshot(
      db,
      `company_product_lifecycle:${action}:${productId}`,
    );
    snapshotPublished = true;
    snapshotVersion = snapshot.version;
  } catch (error) {
    snapshotError =
      error instanceof Error ? error.message : "Storefront snapshot publish failed.";
  }

  await auditRef
    .set(
      {
        storefront_snapshot_status: snapshotPublished ? "published" : "skipped",
        storefront_snapshot_version: snapshotVersion,
        storefront_snapshot_error: snapshotError.slice(0, 500) || null,
        storefront_snapshot_checked_at: FieldValue.serverTimestamp(),
      },
      { merge: true },
    )
    .catch(() => undefined);
...
function isLifecycleAction(
  value: unknown,
): value is CompanyProductLifecycleAction {
