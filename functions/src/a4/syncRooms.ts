...
import { ensureNurseryScopeIndex, resolveA5NurseryScope } from "./nurseryScopeResolver";
...
function isDeviceFallbackRoomLabel(value: string) {
  const normalized = String(value ?? "").trim().toLowerCase();
  return /^device-[0-9a-z_-]+(?:호)?$/.test(normalized);
}
...
async function authorizeRoomSync(request: HttpRequestLike, nurseryId: string, nurseryIdAliases: string[] = []) {
  const authorization = getHeader(request, "authorization");
  const token = authorization.toLowerCase().startsWith("bearer ") ? authorization.slice(7).trim() : "";
  let tokenFailureReason = "";
  const allowedNurseryIds = new Set([nurseryId, ...nurseryIdAliases].filter(Boolean));
  ...
  if (token) {
    try {
      ...
      if (role === "NURSERY_ADMIN" && allowedNurseryIds.has(tokenNurseryId)) {
...
function mapSourceRoom(doc: QueryDocumentSnapshot): SourceRoom | null {
  ...
  const rawRoomIdentity = explicitRoomNumber || explicitRoomName || (looksLikeRoomDocumentId(doc.id) ? doc.id : "");
  const roomNumber = displayRoomNumber(rawRoomIdentity);
  ...
  if (
    (isDevice && !explicitRoomNumber && !explicitRoomName) ||
    isDeviceFallbackRoomLabel(rawRoomIdentity) ||
    isDeviceFallbackRoomLabel(roomNumber)
  ) {
    return null;
  }
...
function mergeRoomDocs(groups: QueryDocumentSnapshot[][]) {
  const merged = new Map<string, QueryDocumentSnapshot>();

  for (const docs of groups) {
    for (const doc of docs) {
      merged.set(doc.ref.path, doc);
    }
  }
...
function sourceRoomDiagnostic(doc: QueryDocumentSnapshot) {
  const data = doc.data();
  const mapped = mapSourceRoom(doc);

  return {
    id: doc.id,
    path: doc.ref.path,
    mapped: Boolean(mapped),
    device_name: fieldString(data, "device_name", "deviceName") || undefined,
    name: fieldString(data, "name") || undefined,
    label: fieldString(data, "label") || undefined,
    title: fieldString(data, "title") || undefined,
    room_number: fieldString(data, "room_number", "roomNumber") || undefined,
    status: sourceStatusString(data) || undefined,
    grade: sourceGradeString(data) || undefined,
    bizNum: fieldString(data, "bizNum", "biz_num") || undefined,
    hasOwnerUid: Boolean(fieldString(data, "owner_uid", "ownerUid")),
  };
}
...
async function readSourceRooms(
  ...
  for (const collectionPath of collectionPaths) {
    const nurseryDocsByField: QueryDocumentSnapshot[][] = [];
    const matchedNurseryFields: string[] = [];
    const businessDocsByField: QueryDocumentSnapshot[][] = [];
    const matchedBusinessFields: string[] = [];
    ...
    if (options.externalNurseryId) {
      ...
      for (const field of nurseryFields) {
        ...
        if (docs.length > 0) {
          nurseryDocsByField.push(docs);
          if (hasUsableRoomDocs(docs)) {
            matchedNurseryFields.push(field);
          }
        }
      }
    }
    ...
    for (const field of businessFields) {
      ...
      if (docs.length > 0) {
        businessDocsByField.push(docs);
        if (hasUsableRoomDocs(docs)) {
          matchedBusinessFields.push(field);
        }
      }
    }

    const mergedDocs = mergeRoomDocs([...nurseryDocsByField, ...businessDocsByField]);
    if (mergedDocs.length > 0 && hasUsableRoomDocs(mergedDocs)) {
      return {
        sourcePath: collectionGroupName ? `collectionGroup:${collectionGroupName}` : collectionPath,
        sourceNurseryField: matchedNurseryFields.length > 0 ? matchedNurseryFields.join("+") : undefined,
        sourceBusinessField: matchedBusinessFields.length > 0 ? matchedBusinessFields.join("+") : undefined,
        docs: mergedDocs,
      };
    }
...
export async function a4LocalRoomUpsertHandler(request: HttpRequestLike, response: HttpResponseLike): Promise<void> {
  ...
  const body = readObjectBody<A4LocalRoomUpsertRequest>(request);
  const requestedNurseryId = optionalString(body.nurseryId);
  const targetDb = getAdminDb();
  const scope = await resolveA5NurseryScope(targetDb, {
    requestedNurseryId,
    businessRegistrationNo: optionalString(body.businessRegistrationNo),
  });
  ...
  if (!nurseryId) {
    sendJson(response, 400, {
      ok: false,
      error: {
        code: "A4_LOCAL_ROOM_UPSERT_INPUT_INVALID",
        message: "nurseryId or businessRegistrationNo is required.",
        httpStatus: 400,
      },
    });
    return;
  ...
  const authorization = await authorizeRoomSync(request, nurseryId, scope.aliasNurseryIds);
  if (!authorization.ok) {
    sendJson(response, 403, {
      ok: false,
      error: {
        code: "A4_LOCAL_ROOM_UPSERT_PERMISSION_DENIED",
  ...
  await ensureNurseryScopeIndex(targetDb, scope);

  const rawRoomNumber = optionalString(body.roomNumber) ?? optionalString(body.name) ?? "";
  const roomNumber = displayRoomNumber(rawRoomNumber) || rawRoomNumber;
  const name = optionalString(body.name) ?? roomNumber;

  if (!roomNumber || !name) {
    sendJson(response, 400, {
      ok: false,
      error: {
        code: "A4_LOCAL_ROOM_UPSERT_ROOM_INVALID",
        message: "roomNumber or name is required.",
        httpStatus: 400,
      },
    });
    return;
  ...
  const roomIdSeed = normalizeRoomNumber(roomNumber) || safeDocumentId(roomNumber, "manual");
  const requestedRoomId = optionalString(body.roomId) ?? "";
  const roomId = safeDocumentId(requestedRoomId, `room-${roomIdSeed}`);
  const existing = await readExistingA5Rooms(targetDb, nurseryId);
  const numberMatch = existing.byNumber.get(normalizeRoomNumber(roomNumber));

  if (numberMatch && numberMatch !== roomId) {
    sendJson(response, 409, {
      ok: false,
      error: {
        code: "A4_LOCAL_ROOM_UPSERT_ROOM_NUMBER_CONFLICT",
        message: "A5 already has a room with the same room number.",
        httpStatus: 409,
      },
    });
    return;
  ...
  const roomRef = targetDb.collection("rooms").doc(roomId);
  const roomSnapshot = await roomRef.get();
  const roomData = roomSnapshot.exists ? roomSnapshot.data() ?? {} : {};
  const existingActiveTabletId = fieldString(roomData, "active_tablet_id", "activeTabletId");
  const activeTabletId =
    optionalString(body.activeTabletId) ||
    existingActiveTabletId ||
    safeDocumentId(`tablet-${roomId.replace(/^room-/, "")}-a`, `tablet-${roomIdSeed}-a`);
  const tabletRef = targetDb.collection("tablets").doc(activeTabletId);
  const tabletSnapshot = await tabletRef.get();
  const tabletData = tabletSnapshot.exists ? tabletSnapshot.data() ?? {} : {};
  const now = FieldValue.serverTimestamp();
  const businessRegistrationNo = scope.businessRegistrationNo || optionalString(body.businessRegistrationNo);
  const resolvedRoomBusinessRegistrationNo =
    businessRegistrationNo || fieldString(roomData, "business_registration_no", "businessRegistrationNo") || null;
  const resolvedTabletBusinessRegistrationNo =
    businessRegistrationNo || fieldString(tabletData, "business_registration_no", "businessRegistrationNo") || null;
  const importSource = fieldString(roomData, "import_source", "importSource") || "A5_LOCAL";
  const floor = optionalString(body.floor) ?? fieldString(roomData, "floor") ?? "";
  const pickupEnabled = typeof body.pickupEnabled === "boolean" ? body.pickupEnabled : fieldBoolean(roomData, true);
  const batch = targetDb.batch();
  ...
  batch.set(
    roomRef,
    {
      ...
      business_registration_no: resolvedRoomBusinessRegistrationNo,
      import_source: importSource,
      local_editable: true,
      status: fieldString(roomData, "status") || "active",
      created_at: roomData.created_at ?? now,
      imported_at: roomData.imported_at ?? now,
      updated_at: now,
    },
    { merge: true },
  );
  ...
  batch.set(
    tabletRef,
    {
      ...
      room_id: roomId,
      label: `${name} tablet`,
      status: fieldString(tabletData, "status") || "active",
      business_registration_no: resolvedTabletBusinessRegistrationNo,
      import_source: fieldString(tabletData, "import_source", "importSource") || importSource,
      created_at: tabletData.created_at ?? now,
      imported_at: tabletData.imported_at ?? now,
      updated_at: now,
    },
    { merge: true },
  );
  ...
  sendJson(response, 200, {
    ...
    nurseryId,
    requestedNurseryId,
    nurseryIdAliases: scope.aliasNurseryIds,
    nurseryScopeResolutionSource: scope.resolutionSource,
    businessRegistrationNo,
    room: {
      targetRoomId: roomId,
      roomNumber,
    ...
    message: "A5 local room was saved without modifying signage-partner.",
  });
}
...
export async function a4RoomsSyncHandler(request: HttpRequestLike, response: HttpResponseLike): Promise<void> {
  ...
  const scope = await resolveA5NurseryScope(targetDb, {
    requestedNurseryId,
    businessRegistrationNo: optionalString(body.businessRegistrationNo),
    externalNurseryId: optionalString(body.externalNurseryId),
  });
  const nurseryId = scope.nurseryId;
  ...
  const businessRegistrationNo = optionalString(body.businessRegistrationNo) || scope.businessRegistrationNo || a5NurseryProfile.businessRegistrationNo;
  ...
  try {
    ...
    sourceRead = await readSourceRooms(sourceDb, {
      externalNurseryId: sourceNursery?.externalNurseryId || optionalString(body.externalNurseryId) || scope.externalNurseryId || a5NurseryProfile.externalNurseryId,
      businessRegistrationNo,
    });
  } catch (error) {
...
export async function a4RoomsListHandler(request: HttpRequestLike, response: HttpResponseLike): Promise<void> {
  setCorsHeaders(response);
  ...
  if (!["GET", "POST"].includes(String(request.method ?? ""))) {
    sendJson(response, 405, {
      ...
      error: {
        ...
        message: "Use GET or POST for A4 room list preview.",
        httpStatus: 405,
      },
    });
    return;
  ...
  const body = readObjectBody<A4RoomsSyncRequest>(request);
  const requestedNurseryId = optionalString(request.query?.nurseryId ?? body.nurseryId);
  const targetDb = getAdminDb();
  const scope = await resolveA5NurseryScope(targetDb, {
    requestedNurseryId,
    businessRegistrationNo: optionalString(request.query?.businessRegistrationNo ?? body.businessRegistrationNo),
    externalNurseryId: optionalString(request.query?.externalNurseryId ?? body.externalNurseryId),
  });
  const nurseryId = scope.nurseryId;
  ...
  if (!nurseryId) {
    sendJson(response, 400, {
      ok: false,
      error: {
        code: "A4_ROOM_LIST_INPUT_INVALID",
        message: "nurseryId or businessRegistrationNo is required.",
        httpStatus: 400,
      },
    });
  ...
  if (!authorization.ok) {
    sendJson(response, 403, {
      ok: false,
      error: {
        code: "A4_ROOM_LIST_PERMISSION_DENIED",
        message: authorization.reason,
        httpStatus: 403,
      },
    });
  ...
  const a5NurseryProfile = await readA5NurseryProfile(targetDb, nurseryId);
  const businessRegistrationNo = optionalString(request.query?.businessRegistrationNo ?? body.businessRegistrationNo) || scope.businessRegistrationNo || a5NurseryProfile.businessRegistrationNo;

  if (!normalizeBusinessRegistrationNo(businessRegistrationNo ?? "")) {
    sendJson(response, 400, {
      ok: false,
      error: {
        code: "A4_ROOM_LIST_BUSINESS_NO_MISSING",
        message: "businessRegistrationNo is required or must exist on the A5 nursery document.",
        httpStatus: 400,
      },
    });
  ...
  try {
    ...
    const sourceRead = await readSourceRooms(sourceDb, {
      externalNurseryId: sourceNursery?.externalNurseryId || optionalString(request.query?.externalNurseryId ?? body.externalNurseryId) || scope.externalNurseryId || a5NurseryProfile.externalNurseryId,
      businessRegistrationNo,
    });
    const existing = await readExistingA5Rooms(targetDb, nurseryId);
    const rooms = sourceRead.docs
      .map((doc) => mapSourceRoom(doc))
      .filter((room): room is SourceRoom => Boolean(room))
      .map((room) => {
        const numberKey = normalizeRoomNumber(room.roomNumber);
        const existingRoomId = existing.byExternalRoomId.get(room.externalRoomId) ?? existing.byNumber.get(numberKey) ?? (existing.byId.has(room.targetRoomId) ? room.targetRoomId : "");
        return {
          ...room,
          activeTabletId: room.activeTabletId || fallbackTabletIdForRoom(room),
          alreadyImported: Boolean(existingRoomId),
          existingRoomId: existingRoomId || undefined,
        };
      });
    const includeDiagnostics = body.includeDiagnostics === true || String(request.query?.includeDiagnostics ?? "") === "true";

    sendJson(response, 200, {
      ok: true,
      mode: "firestore_readonly",
      sourceProjectId,
      sourcePath: sourceRead.sourcePath,
      sourceNurseryPath: sourceNursery?.sourceNurseryPath,
      sourceNurseryField: sourceRead.sourceNurseryField,
      ...
      businessRegistrationNo,
      rooms,
      sourceDiagnostics: includeDiagnostics ? sourceRead.docs.map((doc) => sourceRoomDiagnostic(doc)) : undefined,
      actor: authorization.actor,
      message: "signage-partner rooms were read for A5 import preview. No source data was modified.",
    });
  } catch (error) {
    sendJson(response, 503, {
      ok: false,
      error: {
        code: "A4_ROOM_LIST_SOURCE_READ_FAILED",
        message: error instanceof Error ? error.message : "Unknown signage-partner source read error.",
        httpStatus: 503,
      },
    });
