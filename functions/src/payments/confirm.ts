    ...
    await db.runTransaction(async (transaction) => {
      ...
      transaction.set(
        orderRef,
        {
          ...
          customer_phone: optionalString(body.customerPhone) ?? null,
          status: "paid",
          deliveryMethod: body.deliveryMethod === "delivery" ? "delivery" : "pickup",
          delivery_method: body.deliveryMethod === "delivery" ? "delivery" : "pickup",
          receiver_name: optionalString(body.receiverName) ?? optionalString(body.customerName) ?? "Guest",
          receiver_phone: optionalString(body.receiverPhone) ?? optionalString(body.customerPhone) ?? null,
          receiver_postal_code: optionalString(body.receiverPostalCode) ?? null,
          ...
          delivery_memo: optionalString(body.deliveryMemo) ?? null,
