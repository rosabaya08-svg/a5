  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-normal tracking-[0.12em] text-emerald-600">상품 준수사항</p>
          <h2 className="mt-1 text-lg font-normal text-slate-950">상품 등록 준수사항</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600">
          금지상품, KC 인증 대상 여부, KC 인증번호, 증빙 업로드 상태, 법적 고지 입력 완료 여부를 승인 요청 전에 점검합니다.
          </p>
        </div>
        <span className={`rounded-full px-3 py-1 text-xs font-normal ${gate.approvalRequestEnabled ? "bg-emerald-50 text-emerald-800 ring-1 ring-emerald-200" : "bg-red-50 text-red-700 ring-1 ring-red-200"}`}>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <section className="rounded-md border border-red-200 bg-red-50 p-3">
          <h3 className="font-normal text-red-950">금지상품 체크리스트</h3>
        ...
        <section className="rounded-md border border-amber-200 bg-amber-50 p-3">
          <h3 className="font-normal text-amber-950">KC 인증 / 증빙 조건</h3>
          <div className="mt-3 grid gap-3">
            <label className="flex items-center gap-2 rounded-md bg-white p-3 text-sm font-normal text-slate-800">
              <input
                type="checkbox"
          ...
          <div className="mt-3 grid gap-2">
            {certificationChecklist.map((item) => (
              <div key={item.id} className="rounded-md bg-white p-3 text-xs leading-5 text-slate-600">
                <span className="block text-sm text-slate-950">{item.title}</span>
                {item.description}
              </div>
            ))}
          </div>
      ...
      <section className="mt-4 rounded-md border border-blue-200 bg-blue-50 p-3">
        <h3 className="font-normal text-blue-950">상품 고시 입력</h3>
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {productNoticeFields.map((field) => (
            <label key={field.id} className="grid gap-2 text-sm font-normal text-slate-800">
              <span>
                {field.label}
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <section className="rounded-md bg-slate-50 p-3">
          <h3 className="font-normal text-slate-950">필수 완료 체크</h3>
          <div className="mt-3 grid gap-2">
            {[
              ...
              <label key={key} className="flex items-center gap-2 rounded-md bg-white p-3 text-sm font-normal text-slate-800">
                <input
                  type="checkbox"
        ...
        <section className="rounded-md border border-slate-200 bg-slate-50 p-3">
          <h3 className="font-normal text-slate-950">승인 요청 gate</h3>
          <div className="mt-3 grid gap-2">
            {gate.blockers.length > 0 ? (
              gate.blockers.map((blocker) => (
                <p key={blocker} className="rounded-md bg-red-50 p-3 text-sm font-normal text-red-800">
                  {blocker}
                </p>
              ))
            ) : (
              <p className="rounded-md bg-emerald-50 p-3 text-sm font-normal text-emerald-800">승인 요청 가능 조건을 충족했습니다.</p>
            )}
            {gate.warnings.map((warning) => (
              <p key={warning} className="rounded-md bg-amber-50 p-3 text-sm font-normal text-amber-800">
                {warning}
              </p>
            ))}
          </div>
          <button
            ...
            className="mt-3 w-full rounded-md bg-slate-950 px-4 py-3 text-sm font-normal text-white disabled:cursor-not-allowed disabled:bg-slate-300"
          >
            관리자 승인 요청
          </button>
        </section>
