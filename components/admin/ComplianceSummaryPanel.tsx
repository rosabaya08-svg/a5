  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-normal uppercase tracking-[0.12em] text-red-600">승인 게이트</p>
          <h2 className="mt-1 text-lg font-normal text-slate-950">상품 승인 법적 고지 / KC 검토 요약</h2>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        {summaryCards.map((card) => (
          <div key={card.label} className={`rounded-md border p-3 ${card.tone === "red" ? "border-red-200 bg-red-50" : card.tone === "amber" ? "border-amber-200 bg-amber-50" : "border-blue-200 bg-blue-50"}`}>
            <p className="text-xs font-normal text-slate-500">{card.label}</p>
            <p className="mt-1 text-lg font-normal text-slate-950">{card.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="rounded-md border border-red-200 bg-red-50 p-3">
          <h3 className="font-normal text-red-950">위험 표시 목록</h3>
        ...
        <section className="rounded-md border border-amber-200 bg-amber-50 p-3">
          <h3 className="font-normal text-amber-950">관리자 승인 전 확인</h3>
          <ul className="mt-3 grid gap-2 text-sm text-amber-900">
            <li className="rounded-md bg-white p-3 font-normal">KC 인증 대상이면 인증번호와 증빙이 있어야 합니다.</li>
            <li className="rounded-md bg-white p-3 font-normal">판매자 정보와 CS/반품지 주소가 상품 상세에 표시되어야 합니다.</li>
            <li className="rounded-md bg-white p-3 font-normal">금지상품 의심 항목은 승인 요청 자체를 반려합니다.</li>
            <li className="rounded-md bg-white p-3 font-normal">최종 법률 판단은 전문가 검토 blocker로 유지합니다.</li>
          </ul>
        </section>
      </div>
    </section>
