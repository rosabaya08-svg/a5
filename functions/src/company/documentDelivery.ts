...
async function sendGmailAttachment(uploadId: string, record: CompanyDocumentInboxRecord) {
  ...
  const fallbackRecipient = env("A1_DOCUMENT_GMAIL_TO") || "withcadmin@gmail.com";
  ...
  if (!sendAs || !recipient) {
    await updateDeliveryStatus(uploadId, {
      ...
      gmail_error_code: "gmail_sender_or_recipient_missing",
    });
    return;
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(`Gmail send failed: ${response.status}${detail ? ` ${detail.slice(0, 300)}` : ""}`);
  }
