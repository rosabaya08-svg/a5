...
function platformLabel(value: CompanyApiIntegrationRequest["platformType"]) {
  if (value === "SABANGNET") return "사방넷";
  if (value === "STANDARD") return "기업 표준 API";
  if (value === "ERP") return "ERP";
  if (value === "WMS") return "WMS";
  return "자체 시스템";
}

export function AdminApiIntegrationRequestsPanel() {
  const [requests, setRequests] = useState<CompanyApiIntegrationRequest[]>([]);
  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-normal tracking-[0.14em] text-blue-700">기업 API 요청</p>
          <h2 className="mt-1 text-xl font-normal text-slate-950">기업 API 연동 요청 관리</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600">
            기업 ERP, WMS, 사방넷, 자체 주문 시스템에서 A5 주문/상품/송장 API에 붙기 위해 보낸 요청을 승인하고 배포 상태로 전환합니다.
          </p>
        </div>
      </div>
      ...
      <label className="mt-4 grid gap-2 text-sm font-normal text-slate-800">
        ...
        <textarea
          ...
          className="min-h-20 rounded-md border border-slate-200 px-3 py-3 text-sm font-normal outline-none focus:border-blue-500"
        ...
        {requests.length ? (
          requests.map((request) => (
            <article key={request.id} className="rounded-md border border-slate-100 bg-slate-50 p-3">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs font-normal text-slate-500">
                    {request.companyId} / {request.platformName || platformLabel(request.platformType)}
                  </p>
                </div>
                <span className={`rounded-full px-3 py-1 text-xs font-normal ring-1 ${statusTone(request.status)}`}>
              ...
              {request.deployment ? (
                <div className="mt-3 rounded-md bg-white p-3 text-xs font-normal leading-5 text-slate-700">
          ...
          <p className="rounded-md bg-slate-50 p-4 text-sm font-normal text-slate-600">접수된 API 연동 요청이 없습니다.</p>
        )}
      </div>

      {reviewState.message ? (
        <p className={`mt-4 rounded-md p-3 text-sm font-normal ${reviewState.status === "error" ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-800"}`}>
          {reviewState.message}
        </p>
      ) : null}
    </section>
