...
1. Restore dependencies from the existing `package-lock.json` when install commands are allowed.
2. Re-run `npm.cmd run build` after `node_modules` exists.
3. Run `npm run lint` after dependencies are restored, if allowed.
4. Review route rendering for:
   - `/tablet/products`
   - `/tablet/cart`
   - `/tablet/qr`
   - `/tablet/ask`
...
5. Check whether other worktrees also changed tablet-qr shared data files such as `data/mockProducts.ts`, `data/mockOrders.ts`, `data/mockQrSessions.ts`, or `lib/repositories/mock/**` before merging.
6. Confirm the new tablet/guest panel split is acceptable before extracting more page internals into route-specific components.
7. Review the static search/filter/sort controls and decide which should become real server-backed filters later.
8. Confirm whether the route matrices should remain visible in beta or move to a QA-only/admin-only page later.
9. Review `reports/tablet-qr/BROWSER_CHECK_RESULT.md` before the next browser pass.
10. Review `reports/tablet-qr/SMOKE_CHECKLIST.md` and decide whether any route should be hidden before external beta review.
11. Review `reports/tablet-qr/EDGE_CASE_MATRIX.md` for QR/order edge cases that still need product or policy decisions.
12. Review `reports/tablet-qr/TOUCH_UX_REVIEW.md` before placing the beta on a real room tablet.
13. Use `reports/tablet-qr/BATCH_81_90_PLAN.md` as the next mock-only unattended queue if continuing this track.
14. Review `/tablet/status` in a browser after dev server use is available.
15. Review `reports/tablet-qr/STATUS_ROUTE_MAP.md` and `reports/tablet-qr/STATUS_STATE_COVERAGE.md` alongside the dashboard.
16. Review `reports/tablet-qr/ROUTE_INDEX.md`, `VISUAL_SMOKE_PLAN.md`, and `MERGE_HANDOFF.md` before merge.
