...
export function CompanyProductDraftPreview({ product, options = [] }: CompanyProductDraftPreviewProps) {
    ...
    return (
      <section className="rounded-md border border-slate-200 bg-white p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-600">
          상품 정보를 저장하면 고객에게 보이는 상품 화면을 미리 확인할 수 있습니다.
        </p>
  ...
  const totalStock = draft.variants.reduce((sum, variant) => sum + Number(variant.stock || 0), 0);
  const fulfillmentLabel = draft.fulfillment === "delivery"
    ? "택배 배송"
  ...
  return (
    <div className="grid gap-5">
        ...
        <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
          <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
            <div className="grid aspect-[4/3] place-items-center rounded-md bg-white p-4 text-center text-sm font-normal text-slate-400">
              {representativeUrl ? (
                <img src={representativeUrl} alt={draft.productName || "상품 대표 이미지"} className="h-full w-full rounded-md object-contain" loading="lazy" />
              ) : (
          ...
          <div className="grid content-start gap-3">
            {[
              ["판매 상태", draft.saleStatus],
              ["카테고리", `${draft.categoryLabel} / ${draft.subcategory}`],
              ["폐쇄몰 판매가", formatCurrency(draft.pricing.closedMallPrice)],
              ["재고", `${totalStock.toLocaleString()}개`],
              ["배송", fulfillmentLabel],
              ["원판매가", comparisonVerified ? formatCurrency(draft.pricing.listPrice) : "확인 전"],
              ["오픈몰 판매가", comparisonVerified ? formatCurrency(draft.pricing.platformLowestPrice) : "확인 전"],
              ["AI 가격 비교", comparisonVerified ? `${draft.pricing.normalDiscountRate}% 할인` : "확인 전"],
              ["옵션", `${draft.variants.length}개`],
            ].map(([label, value]) => (
      ...
      <section className="rounded-md border border-slate-200 bg-white p-5">
        <h3 className="text-lg font-normal text-slate-950">옵션별 판매정보</h3>
        <div className="mt-4 overflow-x-auto rounded-md border border-slate-200">
          <table className="min-w-[720px] border-collapse text-left text-sm">
            <thead className="bg-slate-50 text-xs font-normal text-slate-500">
              <tr>
                {["옵션", "추가금", "최종 판매가", "재고", "판매 상태"].map((item) => (
            ...
            <tbody className="divide-y divide-slate-100">
              {draft.variants.map((variant) => {
                const finalSalePrice = Number(variant.finalSalePrice || 0);
                return (
                  <tr key={variant.id}>
                    <td className="px-4 py-3 font-normal text-slate-950">{variant.optionPath || "기본"}</td>
                    <td className="px-4 py-3">{formatCurrency(Number(variant.additionalPrice || 0))}</td>
