...
const preservedTestPassword = "1004";
const legacyBusinessNumberPasswordPolicy = "business_registration_number_initial";
const temporaryPasswordPolicy = "temporary_1004_force_change";
...
const preservedTestAdminAccount: ResolvedCompanyAccount & { password: string } = {
  companyId: "company-test-1004",
  businessNo: registeredClosedMallBusinessNo,
  password: preservedTestPassword,
  displayName: "7592901311 테스트 관리자",
  status: "approved",
  passwordPolicy: temporaryPasswordPolicy,
...
export async function companyBetaAuthTokenHandler(request: HttpRequestLike, response: HttpResponseLike): Promise<void> {
  ...
  const preservedAccount = resolvePreservedTestAdminAccount({ companyId, businessNo });
  const account = preservedAccount && preservedAccount.password === password
    ? preservedAccount
    : await resolveCompanyAccount({ companyId, businessNo });

  if (!account) {
    sendJson(response, 403, { ok: false, resultCode: 403, resultMsg: "등록된 사업자 계정을 찾을 수 없습니다." });
    return;
  }
  if (isBlockedStatus(account.status)) {
    sendJson(response, 403, { ok: false, resultCode: 403, resultMsg: "현재 사용할 수 없는 기업 계정입니다. 최고관리자에게 문의해 주세요." });
    return;
  }

  const usesLegacyBusinessNumberPassword = account.passwordPolicy === legacyBusinessNumberPasswordPolicy;
  const passwordMatches = usesLegacyBusinessNumberPassword
    ? password === preservedTestPassword
    : verifyCompanyPassword({ password, passwordHash: account.passwordHash, legacyPassword: account.password });
  if (!passwordMatches) {
    sendJson(response, 403, { ok: false, resultCode: 403, resultMsg: "사업자등록번호 또는 비밀번호가 일치하지 않습니다." });
    return;
  }
  if (usesLegacyBusinessNumberPassword) await migrateLegacyInitialPassword(account);
  ...
  try {
    ...
    sendJson(response, 500, { ok: false, resultCode: 500, resultMsg: "기업관리자 로그인 토큰 발급 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요." });
  }
}

function resolvePreservedTestAdminAccount(input: { companyId: string; businessNo: string }) {
  if (normalizeBusinessNo(preservedTestAdminAccount.businessNo) !== input.businessNo) return null;
  if (input.companyId && input.companyId !== preservedTestAdminAccount.companyId) return null;
  return preservedTestAdminAccount;
}
...
async function resolveCompanyAccount(input: { companyId: string; businessNo: string }): Promise<ResolvedCompanyAccount | null> {
  ...
  if (firestoreAccount && (!input.companyId || firestoreAccount.companyId === input.companyId)) return firestoreAccount;
  return resolvePreservedTestAdminAccount(input);
}
...
async function findCompanyByBusinessNo(businessNo: string): Promise<ResolvedCompanyAccount | null> {
  ...
  return {
    ...
    displayName: normalizedBusinessNo === registeredClosedMallBusinessNo
...
async function findCompanyByRawBusinessNo(businessNo: string) {
  ...
  return scan.docs.find((document) => {
    const data = document.data();
...
async function migrateLegacyInitialPassword(account: ResolvedCompanyAccount): Promise<void> {
  ...
  await db.runTransaction(async (transaction) => {
    transaction.set(db.collection("companies").doc(documentId), {
      company_login_password_hash: hashCompanyPassword(preservedTestPassword),
