...
import {
  buildJandiroPricePolicy,
  JANDIRO_PRICE_POLICY_ID,
  shouldApplyJandiroPricePolicy,
  type CompanyProductOperation,
} from "./jandiroPricePolicy";
...
export async function companyProductUpsertHandler(request: HttpRequestLike, response: HttpResponseLike): Promise<void> {
  ...
  try {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      const operation = text(body.operation, "publish") as CompanyProductOperation;
      const policyApplies = shouldApplyJandiroPricePolicy(requestedCompanyId, product);
      const jandiroPolicy = policyApplies
        ? buildJandiroPricePolicy(
            product,
        ...
        : undefined;
      if (policyApplies && !jandiroPolicy) {
        throw new Error("JANDIRO_PRICE_POLICY_SOURCE_PRICE_REQUIRED");
      }
      const pricePolicyFields = jandiroPolicy?.fields ?? {};


      const normalizedProduct = {
        ...productDocument(
          { ...product, ...pricePolicyFields },
          productId,
          requestedCompanyId,
          company,
          now,
      ...
      if (Object.keys(supplyProfile).length) {
        const normalizedSupplyProfile = sanitize({
          ...supplyProfile,
          ...(jandiroPolicy ? { shipping_fee: 3_000 } : {}),
          id: productId,
      ...
      transaction.set(db.collection("audit_logs").doc(`company-product-upsert-${productId}-${Date.now()}`), {
        ...
        price_policy_id: jandiroPolicy ? JANDIRO_PRICE_POLICY_ID : null,
        price_policy_source_price: jandiroPolicy?.sourcePrice ?? null,
        price_policy_applied: Boolean(jandiroPolicy),
    ...
    try {
      const snapshot = await publishStorefrontRuntimeSnapshot(
        db,
        `company_product_${text(body.operation, "publish")}:${productId}`,
      );
      ...
      snapshotWarning =
        snapshotError instanceof Error
          ...
          : "Storefront snapshot publish failed.";
      await db
        .collection("audit_logs")
        .doc(`company-product-snapshot-failed-${productId}-${Date.now()}`)
        .set({
          type: "company_product_snapshot_publish_failed",
          action: text(body.operation, "publish"),
          product_id: productId,
          company_id: requestedCompanyId,
          actor_uid: claims.uid,
          public_storefront_source: "products_collection_via_public_function",
          error_message: snapshotWarning,
          created_at: FieldValue.serverTimestamp(),
        });
    }
