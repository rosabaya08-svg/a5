  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-4">
        <h3 className="text-base font-normal text-slate-950">주의 항목</h3>
        <span className="text-[11px] font-normal tracking-[0.14em] text-slate-400">위험</span>
      </div>
      <div className="mt-4 grid gap-2">
        {risks.map((risk) => (
          <article key={risk.id} className={`rounded-md border p-3 ${severityClasses[risk.severity]}`}>
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="font-normal">{risk.title}</p>
              <span className="text-xs font-normal">{severityLabels[risk.severity]} · {risk.owner}</span>
            </div>
