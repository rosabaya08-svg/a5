...
async function resolveUser(body: Record<string, unknown>) {
  ...
  if (requestedUid) return getAdminAuth().getUser(requestedUid);
...
export const payupAdminAccess = onRequest(options, async (request, response) => {
  try {
    ...
    const approvalRequestId = text(body.approvalRequestId, 200);
    ...
    const approvalPayload = { uid: target.uid, roles, status, organizationIds, businessNumbers, channelIds };
    ...
    response.status(200).json({ ok: true, uid: target.uid, roles, status, correlationId });
  } catch (error) {
    sendAccessError(response, error);
  }
});

export const payupAdminApprovals = onRequest(options, async (request, response) => {
  try {
    const body = asRecord(request.body);
    const action = text(body.action, 30) || "list";
    ...
    if (action === "request") {
      ...
      response.status(200).json({ ok: true, requestId, status: "PENDING" });
      return;
    }
    ...
    const requestId = safeDocumentId(text(body.requestId, 200));
